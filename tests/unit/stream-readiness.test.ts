...
test("ensureStreamReadiness surfaces Devin Desktop resource_exhausted as terminal 429", async () => {
  const warnings: string[] = [];
  const response = new Response(
    streamFromChunks([
      `data: ${JSON.stringify({
        error: {
          message:
            "Devin Desktop stream error: resource_exhausted: Reached overall message " +
            "rate limit. Please try again later. Your limit will reset in 16 minutes.",
        },
      })}\n\n`,
    ]),
    { status: 200, headers: { "Content-Type": "text/event-stream" } }
  ...
  assert.match(result.reason, /resource_exhausted/i);
});

test("ensureStreamReadiness surfaces context-length diagnostics as terminal 400", async () => {
  const response = new Response(
    streamFromChunks([
      `data: ${JSON.stringify({
        error: {
          message:
            "Devin Desktop stream error: invalid_request_error: Your prompt is too long: " +
            "263045 tokens > 262000 maximum context length.",
        },
      })}\n\n`,
    ]),
    { status: 200, headers: { "Content-Type": "text/event-stream" } }
